...
if ($_GET['action'] == "list") {
    ...
    $sql = mysqli_query($conn, "INSERT INTO receipt_list (id_product, pd_name,  price, count, date, time, sumprice, sumcount, money_input, money_change)
    VALUES ('$id_product', '$pd_name', ' $price', '$count', '$date_now_sql', '$time_now_sql', '$sumprice', '$sumcount', '$money_input', '$money_change')");

    for ($i = 0; $i < count($idArr); $i++) {
        $sql = mysqli_query($conn, "UPDATE product SET stock = stock - $countArr[$i]
        WHERE id = $idArr[$i]");
    }
